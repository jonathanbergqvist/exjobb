...
double InterfaceComparison::superimpose(Molecule* m1, Molecule* m2) {
...
void InterfaceComparison::transpose_matrix(double** m) {/* Transpose a 3x3 matrix */
	double dummy;
	dummy = m[0][1];
	m[0][1] = m[1][0];
	m[1][0] = dummy;
...
void InterfaceComparison::multiply_matrix(double** a, double** b,
...
void InterfaceComparison::copy_matrix(double** f, double** t){ /* copy matrix f into matrix t */
...
void InterfaceComparison::cacheDifference(){
	...
	for(int i=0; i<this->length; i++){
		...
		//this->contributes++;
		if(!molA->getNull()[i])
			tempSeqScore += BLOSUM[AAtoi[molA->getSeq()[i]-'A']][AAtoi[molB->getSeq()[i]-'A']];
...
void InterfaceComparison::differencePerResidue(){
	...
	for(int i=0; i<this->length; i++){
		...
		for(int j=i; j<this->length; j++){
				...
				if(!molA->getNull()[i] && !molA->getNull()[j]){
					...
					this->diffCumulative[i] += abs(molA->getDist()[i][j]-molB->getDist()[i][j]);
...
void InterfaceComparison::deltaA(int i, int j){
...
void InterfaceComparison::deltaSeqA(int i, int j){
	...
	//if i is going to be enabled (j disabled)
	if(molA->getNull()[i]){
	  de += - (BLOSUM[AAtoi[molA->getSeq()[i]-'A']][AAtoi[molB->getSeq()[i]-'A']]  - BLOSUM[AAtoi[molA->getSeq()[j]-'A']][AAtoi[molB->getSeq()[j]-'A']]);
		//de = BLOSUM[AAtoi[molA->getSeq()[j]-'A']][AAtoi[molB->getSeq()[j]-'A']] - BLOSUM[AAtoi[molA->getSeq()[i]-'A']][AAtoi[molB->getSeq()[i]-'A']];
...
void InterfaceComparison::deltaB(int i, int j){
...
void InterfaceComparison::deltaSeqB(int i, int j){
	...
	if(j < this->length){
			...
			if(!molA->getNull()[i]){//if the difference is lower, delta has to be negative   //
				...
				de += - (BLOSUM[AAtoi[molA->getSeq()[i]-'A']][AAtoi[molB->getSeq()[j]-'A']] - BLOSUM[AAtoi[molA->getSeq()[i]-'A']][AAtoi[molB->getSeq()[i]-'A']]);
			...
			if(!molA->getNull()[j]){//if the difference is lower, delta has to be negative //
				             ...
				             //new difference   	 //old difference
				de+= - (BLOSUM[AAtoi[molA->getSeq()[j]-'A']][AAtoi[molB->getSeq()[i]-'A']] - BLOSUM[AAtoi[molA->getSeq()[j]-'A']][AAtoi[molB->getSeq()[j]-'A']]);
...
double InterfaceComparison::getsScore(){

	return this->sScore;

}

double** InterfaceComparison::getRotation(){

	return this->s;

}

double** InterfaceComparison::getI(){

	return this->I;

}

double* InterfaceComparison::getTranslation1(){

	return this->trans1;

}

double* InterfaceComparison::getTranslation2(){
...
double* InterfaceComparison::getDiffCumulative(){
...
void InterfaceComparison::printBLOSUM62(){

	for(int i=0; i<24; i++){
		for(int j=0; j<24; j++){

			cout << BLOSUM[i][j] << " " << flush;
